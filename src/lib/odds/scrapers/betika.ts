...
export async function scrapeBetika(): Promise<ScrapedOdds[]> {
  ...
  for (const tab of TABS) {
    ...
    for (const r of rows) {
      ...
      // Filter Simulated Reality League matches (RNG, never real): Betika
      // marks them either via `is_srl` OR by suffixing team names with "Srl".
      // provider === "sr" means Sport Radar as data source, NOT SRL — real
      // Chelsea/UEFA fixtures come in with provider "sr" too.
      if (r.is_srl) continue;
      if (/\bsrl\b/i.test(r.home_team) || /\bsrl\b/i.test(r.away_team)) continue;
